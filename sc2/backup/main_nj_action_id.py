...
from model import FullyConvSelecAction
from optimizer import SharedAdam
from train_neural_junction import train_neural_junction_action_id
...
def main(argv):
    ...
    # share model
    source_model = FullyConvSelecAction(screen_channels=8, screen_resolution=(
        FLAGS.screen_resolution, FLAGS.screen_resolution)).cuda()
    source_model.load_state_dict(torch.load('./models/task1_extended_action_id_26581299/model_best'))

    shared_model = FullyConvSelecAction(screen_channels=8, screen_resolution=(
            FLAGS.screen_resolution, FLAGS.screen_resolution)).cuda()
    ...
    shared_model.non_spatial_policy.load_state_dict(source_model.non_spatial_policy.state_dict())
    ...
    freeze_layers(shared_model.non_spatial_policy)
    ...
    for worker_id in range(FLAGS.num_of_workers):
        worker = mp.Process(target=train_neural_junction_action_id, args=(
                worker_id, FLAGS.flag_values_dict(), shared_model, optimizer, global_counter, summary_queue))
        worker.start()
