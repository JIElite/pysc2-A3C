...
from model import FullyConv, FullyConvSelecAction, FullyConvExtended
...
def train_neural_junction_action_id(worker_id, args, shared_model, optimizer, global_counter, summary_queue):
    torch.manual_seed(args['seed'] + worker_id)
    env_args = {
        'map_name': args['map'],
        'agent_race': args['agent_race'],
    ...
    with env:
        ...
        freeze_layers(local_model.spatial_policy)
        freeze_layers(local_model.non_spatial_branch)
        freeze_layers(local_model.non_spatial_policy)
        freeze_layers(local_model.value)
...
def train_extended_nj(worker_id, args, shared_model, optimizer, global_counter, summary_queue):
    torch.manual_seed(args['seed'] + worker_id)
    env_args = {
        'map_name': args['map'],
        'agent_race': args['agent_race'],
    ...
    game_inferface = GameInterfaceHandler(screen_resolution=args['screen_resolution'], minimap_resolution=args['minimap_resolution'])
    with env:
        local_model = FullyConvExtended(screen_channels=8, screen_resolution=[args['screen_resolution']]*2).cuda()
